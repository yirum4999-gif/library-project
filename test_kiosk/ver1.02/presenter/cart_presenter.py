...
class CartPresenter:
    ...
    def scan_book(self, barcode):
        ...
        if self.mode == "loan":
            ...
            returned = book.get("returnable", "可")
            overdue = book.get("overdue", "なし")
            row = (book.get("title", "不明な書名"), f"返却：{returned}\n延滞：{overdue}")
        ...
        if len(self.barcodes) >= 10:
            ...
            InfoDialog("10冊になりました", f"10冊を{verb}します。確認へ進んでください。", self.view).exec()
            self.confirm()

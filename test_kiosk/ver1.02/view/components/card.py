...
class ActionCard(QFrame):
    ...
    def __init__(self, title, color_theme="#2563EB", card_w_size=320,
                 card_h_size=None, subtitle=None, parent=None, title_font_size=27):
        ...
        # 호출하지 않으면 27pt를 사용하며, 카드 높이에 맞춰 생성 시 조정할 수 있다.
        self.title.setFont(QFont("Yu Gothic UI", title_font_size, QFont.Weight.Bold))
        self.title.setWordWrap(True)
        layout = QVBoxLayout(self)
        # 키패드처럼 낮은 카드에서도 문자가 상하로 잘리지 않도록 여백을 줄인다.
        padding = 6 if self.height() <= 80 else 24
...
class ManualInputCard(ActionCard):
    ...
    def __init__(self, parent=None):
        super().__init__("会員番号を入力", "#D97706", 470, 112,
                         "バーコードを読み取れない場合", parent,title_font_size=18)

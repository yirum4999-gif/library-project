...
class ScanLoginPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.cancel_card = ActionCard("取消", "#64748B", 210, 112)
        ...
        bottom_actions.addWidget(self.manual_input_card)
        bottom_actions.addWidget(self.cancel_card)
        bottom_actions.addStretch()
...
class ManualInputPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for index, key in enumerate(keys):
            keypad.addWidget(self._key(key), index // 6, index % 6)
        keypad.addWidget(self._key("削除"), 6, 0, 1, 3)
        keypad.addWidget(self._key("全消去"), 6, 3, 1, 3)
        # 6개 열의 실제 버튼 폭을 고정해 넓은 화면에서도 간격이 벌어지지 않는다.
        keypad_box.setFixedSize(558, 436)
        ...
        layout = QVBoxLayout(self)
        layout.setContentsMargins(180, 35, 180, 35)
        layout.addWidget(title)
        layout.addSpacing(18)
        layout.addWidget(self.member_input)
        layout.addSpacing(16)
        layout.addWidget(keypad_box, alignment=Qt.AlignmentFlag.AlignCenter)
        layout.addSpacing(12)
        layout.addWidget(self.action_card)

    def _key(self, text):
        special = text in ("削除", "全消去")
        card = ActionCard(text, "#334155" if special else "#2563EB", 272 if special else 88, 58,
                          title_font_size=18 if special else 20)
        card.clicked.connect(lambda: self._press(text))
        return card

    def _press(self, text):
        value = self.member_input.text()
        if text == "削除":
            self.member_input.setText(value[:-1])
        elif text == "全消去":
            self.member_input.clear()
        elif len(value) < self.member_input.maxLength():
            self.member_input.setText(value + text)
